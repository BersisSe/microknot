use feather::{Finalizer, Response, Router, State, end, json, middleware, next};
    ...
    router.post(
        ...
        middleware!(|req, res, ctx| {
            ...
            match result {
                ...
                Err(CoreError::Duplicate { id }) => send_json(
                    res,
                    409,
                    &json!({"error": format!("workflow '{id}' already exists")}),
                ),
                Err(e) => internal_error(res, &e),
            }
        }),
...
fn validation_error(res: &mut Response, errors: Vec<ValidationError>) -> feather::Outcome {
    ...
    send_json(
        res,
        422,
        &json!({"error": "invalid workflow", "details": details}),
    )
}
